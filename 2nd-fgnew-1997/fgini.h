 ...
 * $Id: fgini.h,v 2.2 1996/11/24 23:47:15 korpela Exp korpela $
 */
...
extern FG_INT32 next_entry;

typedef struct _FG_INI_ENTRY {
  FG_INT32  struct_size;           /* size of this structure       */
  FG_CHAR filename[32];          /* file that driver is in       */
  FG_CHAR driver_name[32];       /* name of the driver           */
  FG_INT32  autoload;              /* automatically load driver    */
  FG_INT32  has_bitmap;            /* driver has a bitmap          */
  FG_INT32  window_number;         /* which window is bitmap in    */
  FG_INT32  x,y,width,height;      /* position and size of bitmap  */
  FG_INT32  flags;                 /* flags used by driver         */
} FG_INI_ENTRY;
...
FG_INT32 read_ini_file(void);
FG_INT32 write_ini_file(void);
