 ...
 * $Id: fgmain.c,v 2.2 1996/11/24 23:47:15 korpela Exp korpela $
 */
...
FG_INT32 next_driver=0;
FG_BITMAP *driver_bitmap[MAXDRIVERS];

FG_INT32 master_event, /* called every time through the main loop           */
		 idle_event,   /* called at end of main loop is there's time before */
							/* next clock tick                                   */
		 exit_event;   /* called before exit                                */

FG_PUBVARS *public_vars; /* pointer to public variable space              */

#if defined(USE_WIN32)

HINSTANCE my_hInstance, my_hPrevInstance;
FG_INT32 my_nCmdShow;

FILE *output;

#if defined(OPEN32)
int main(FG_INT32 argc, char *argv[])
{
...
FG_INT32 allocate_public_vars()
{
  return (!(public_vars=(FG_PUBVARS *)calloc(PUBLIC_VARS_SIZE,1)));
}

FG_INT32 load_static_drivers() 
{
      ...
      if (ini_entry->autoload) {
        if (ini_entry->has_bitmap) {
          if (ini_entry->window_number) {
            ...
            callstruct.bitmap=
              fg_create_bitmap(ini_entry->width, ini_entry->height,display_bitmaps[0]->color_format);
          }
...
FG_INT32 load_external_drivers() 
{
  FGDRIVER_INIT_CALL callstruct;
  FG_INT32 i=0;
  FG_INI_ENTRY *ini_entry;
...
{
  time_t clock1;
  FGEVENT_CALL master_call,idle_call,exit_call;
  master_call.event_id=master_event;
  idle_call.event_id=idle_event;
  exit_call.event_id=exit_event;
