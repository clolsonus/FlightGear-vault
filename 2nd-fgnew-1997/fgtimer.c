 ...
 * $Id: fgtimer.c,v 2.2 1996/11/24 23:47:15 korpela Exp korpela $
 */
...
struct _FG_TIMER {
  FG_INT32 event;     /* event id */
  FG_INT32 counter;   /* remaining time */
  FG_INT32 value;     /* timer count value */
  char name[MAX_FGEVENT_NAME]; /* event name */
} timer[NUM_TIMERS];

FG_INT32 master_timer,timer_incr;
FG_INT32 n_timers;
struct timeb timer_time0,timer_time1;
FG_INT32 timer_paused;
FG_PUBVARS *timer_public_vars;
...
#define public_vars timer_public_vars
	 ...
	 timer_incr=-1;
	 while ((timer_incr<0) || (timer_incr>100)) {
		ftime(&timer_time0);
	 ...
	 fprintf(stderr,"timer_incr=%d ms\n",timer_incr);

	 AC_TIME_MS=(timer_time0.time % 86400L)*1000L+timer_time0.millitm;
  }
...
FG_VOID *fgtimer_master(FGEVENT_CALL *my_event)
{
  ...
  elapsed=(timer_time1.time-timer_time0.time)*1000L+
...
FG_INT32 handle_fgtimer_event(FG_INT32 time_ms,FG_EVENT_FN function)
{
  FG_INT32 i,diff,mindiff=10000L,besttimer=-1;
  ...
  if (((mindiff*16L)<time_ms) || (n_timers==NUM_TIMERS)) {
...
FG_INT32 unhandle_fgtimer_event(FG_INT32 time_ms,FG_EVENT_FN function)
{
  FG_INT32 i,diff,mindiff=10000L,besttimer=-1;
  char name[MAX_FGEVENT_NAME];
  time_ms=FG_MAX(timer_incr,((time_ms+timer_incr/2)/timer_incr)*timer_incr);
