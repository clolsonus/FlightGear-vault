 ...
 * $Id: fgkbd.c,v 2.2 1996/11/24 23:47:15 korpela Exp korpela $
 */
...
volatile FG_INT32 kbd_buffer[128];
volatile FG_INT32 kb_read_ptr=0,kb_write_ptr=0;
#endif
...
FG_INT32 fg_define_key(char *event_name);
FG_INT32 fg_keymap_changed=0;
...
FG_VOID *fgkbd_init(FGDRIVER_INIT_CALL *callstruct)
{
  ...
  #ifdef USE_ALLEGRO
    if (install_keyboard()) {
       fprintf(stderr,"Error Installing Keyboard Handler\n");
    }
  #elif defined(USE_FB)
    { 
      if (win=initscr()) {
...
FG_INT32 fg_GetPMKey(MPARAM mp1, MPARAM mp2)
{
  ...
  FG_INT32 key=SHORT1FROMMP(mp2);
  FG_INT32 v_key=SHORT2FROMMP(mp2);
...
FG_INT32 fg_define_key( char *event_name )
{
  ...
  fprintf(stderr,"Press a key for the %s event\n",event_name);
#ifdef USE_ALLEGRO
...
FG_INT32 handle_fgkey_event(char *event_name,FG_EVENT_FN function)
{
  ...
  while (!code) {
	 fprintf(stderr,"Event (%s) has no mapped key\n",event_name);
    code=fg_define_key(event_name);
    for (i=0;i<nkeys;i++) {
      if (code==keys[i].key_code) {
		  fprintf(stderr,"Key 0x%x is already mapped to event %s.\n",
  ...
  if (nkeys<NUM_KEY_EVENTS) {
    ...
    return(keys[nkeys++].event_code);
	 ...
	 fprintf(stderr,"Out of keymap slots!\n");
	 return(0);
  }
}
