...
int	ptblCount;	/* number of active entries in ptbl */
craft	ptbl[MAXPLAYERS]; /* table of player aircraft */
...
double calcRho (double alt, double *mach) {
    double	deltaAlt, b;
...
void twoOrder (double k, double d, double y, double v, double *newy,
	       double *newv) 
{
...
void calcCoefficients (craft *c, double *CLift, double *CDrag) {
...
void craftToGround (craft *c, VPoint *p, VPoint *g) {
    VTransform (p, &(c->trihedral), g);
}

void calcGForces (craft *c, VPoint *f, double w) {
    VPoint t;
...
void calcAlphaBeta (craft *c, double *alpha, double *beta) {
...
/* buildEulerMatrix : Build a transformation matrix based on the
...
double elevatorSetting (craft *c, double q, double w) {
...
int flightCalculations (craft *c) {
    ...
    /* Compute the resultant force vector on the aircraft.  By the
     * way, the variable "q" should more properly be named "qS" -- it
     * is the dynamic pressure times S, the reference wing area.  */
    ...
    /* These expressions convert lift and drag forces from wind axes
     * to the aircraft fixed axes.  The conversion is based on the
     * wind to aircraft transformation matrix supplied in "Airplane
     * Design" by Donald Crawford (page 90).  */
	...
	/* groundDynamics handles movement when we're in contact with the
	 *  earth.  */
	...
	/* Nose wheel steering is only active when we cannot lift off
         * -- cancel z */
	 ...
	 *
	 * We do some trickery here.  If the absolute value of the
	 * sideslip angle is greater than 90 degrees, we trick the
	 * code into believing that the sideslip angle is the negative
	 * of its reciprocal value (e.g. -176 becomes -4 degrees).  We
	 * do this with the (somewhat inaccurate) assumption that the
	 * CN value for that angle is roughly equal to the other.  */
...
deg/sec, r = %g deg/sec\n", RADtoDEG(c->p), RADtoDEG(c->q), RADtoDEG(c->r));
	 ...
	 *  Post processing
	 *
	 *  Check to see if we make ground contact during this
	 *  interval; if so, recalculate the aircraft position taking
	 *  into account the ground dynamics after touchdown.  */
