 ...
 * $Id: fgscreen.c,v 2.1 1996/10/31 18:12:04 korpela Exp korpela $
 */
#include "fgscreen.h"
#include <stdio.h>
#include <stdlib.h>
...
int allocate_display_bitmaps()
{
...
#ifdef USE_FB
  {
    ...
    if (strcmp(ttyname(0),"/dev/console")) {
      printf("You must be logged into the console to use /dev/fb\n");
      exit(1);
    }
    if ((fb_fd=open("/dev/fb", O_RDWR))==-1) {
      printf("Unable to open /dev/fb\n");
      exit(1);
    }
    if (ioctl(fb_fd,FBIOGATTR,&fb)) {
      if (ioctl(fb_fd,FBIOGTYPE,&fb.fbtype)) {
        printf("Unable to determine fb type.\n");
        exit(1);
      }
    }
    if (fb.fbtype.fb_depth != 8) {
      printf("Only 8 bit color is availbable at this time.\n");
      exit(1);
    }
    ...
    display_bitmaps[0]=(FG_BITMAP *)calloc(1,
	     sizeof(FG_BITMAP)+fb.fbtype.fb_height*sizeof(FG_UINT8 **));
    ...
    {
      printf("Unable to map frame buffer to process memory.\n");
      exit(1);
    }
    for (i=1;i<fb.fbtype.fb_height;i++) {
      display_bitmaps[0]->lines[i]=(display_bitmaps[0]->lines[i-1])+fb.fbtype.fb_width;
    }
...
#ifdef __DJGPP__
  {
    int modes_x[]={ 320, 640, 640, 800, 1024, 1280, 0 };
    int modes_y[]={ 200, 400, 480, 600,  768, 1024, 0 };
    int i=0,closest_mode,diff=INT_MAX,mode_diff;
    ...
    if (allegro_init() ||
       !(display_bitmaps[0]=fg_create_bitmap(scn_width,scn_height)) ||
    ...
    fg_set_startup_palette();
    display_alias=malloc(sizeof(BITMAP)+sizeof(char *)*scn_height);
    display_alias->w=display_bitmaps[0]->w;
    display_alias->h=display_bitmaps[0]->h;
    display_alias->clip=display_bitmaps[0]->clip;
    display_alias->cl=display_bitmaps[0]->cl;
    display_alias->cr=display_bitmaps[0]->cr;
    display_alias->ct=display_bitmaps[0]->ct;
    display_alias->cb=display_bitmaps[0]->cb;
    display_alias->dat=display_bitmaps[0]->lines[0];
    display_alias->seg=_my_ds();
    display_alias->write_bank=_stub_bank_switch;
    display_alias->read_bank=_stub_bank_switch;
    for (i=0;i<display_alias->h;i++) display_alias->line[i]=display_bitmaps[0]->lines[i];
  }
#endif
...
/* MOVE THIS SECTION INTO MESSAGE THREAD WHEN WE ADD WIN32 THREADING */

  if (!my_PrevInstance) 
  {
    fg_WndClass.style=CS_HREDRAW | CS_VREDRAW;
    fg_WndClass.lpfnWndProc=FG_WndProc;
    fg_WndClass.cbClsExtra=0;
    fg_WndClass.cbWndExtra=0;
    fg_WndClass.hInstance=my_hInstance;
    fg_WndClass.hIcon=LoadIcon(NULL,IDI_APPLICATION);
    fg_WndClass.hCursor=LoadCursor(NULL,INI_ARROW);
    fg_WndClass.hbrBackground=GetStockObject(BLACK_BRUSH);
    fg_WndClass.lpszMenuName=MenuName;
  ...
  ShowWindow(fg_hwnd, nCmdShow);
  UpdateWindow(fg_hwnd);
  handle_fgevent("master",FG_message_thread);
#endif
