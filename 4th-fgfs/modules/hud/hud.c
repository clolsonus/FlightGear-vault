...
double magHeading();
extern void fgDrawSegments (Segment *seg, int nseg, int color);
FG_BITMAP *fghud_bmp;
...
void VDrawArc (int x, int y, int width, int height, int angle1, 
	       int angle2, int color)
{
}


/* magnetic variation is hardwired in for now */
double mag_var = DEGtoRAD (8.0);  /* eight degrees east */

int hud_color = 1;

void doHUD (craft  *c, viewer *u) {
 ...
 */
	...
	/* Our horizon structure is a list of point paths on a unit
	 * circle.  Traverse that structure, transforming each set of
	 * points into screen coordinates, and add a segment entry for
	 * each one that'll be displayed.  */
 ...
 */
	...
	/* Let's not do this for now.  Maybe later we can add this back in */
	/* doWeaponDisplay (c, u); */
...
/* radar target is assumed to be ahead of us (tmp.z > 0.0) */
	...
	fgDrawSegments (seg, i, hud_color);

	VDrawArc (u->xCenter-2, u->yCenter-2, 4, 4, 0, 360*64,
		u->v->pixel[HUDPixel]);

	if (plotFPM) {
		a4 = 16.0 * u->scaleFactor + 0.5;
		a5 = (a4 + 1) / 2;
		VDrawArc (windX-a5, windY-a5, a4, a4, 0, 360*64,
			u->v->pixel[HUDPixel]);
	}
...
/* calculate magnetic heading from true heading (currently uses a
   hardwired offset */

double magHeading (craft *c) {
    register double h = c->curHeading - mag_var;
    return (h < 0.0) ? h + 2.0 * M_PI : h;
}


/* draw out all the lines in a "Segment" structure */

void fgDrawSegments (Segment *seg, int nseg, int color) {
    for ( ; nseg > 0; -- nseg) {
	color=random() % 16;
	fg_line(seg->x1, seg->y1, seg->x2, seg->y2, color, fghud_bmp);
	++seg;
    }
}
