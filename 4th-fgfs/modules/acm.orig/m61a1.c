 ...
 */
    ...
    m = (double) BORE_CROSS_SIZE *u->scaleFactor + 0.5;
 ...
 */
    ...
    fuzz = ((double) (acm_rand() & 32767) +
	    (double) (acm_rand() & 32767)) / 32768.0 - 1.0;
    fuzz = fuzz * FUZZ;
    fuzzAngle = 2.0 * pi * (double) (acm_rand() & 511) / 512.0;
...
#ifdef HAVE_DIS
    dis_fire_cannon (c, &m->Sg, &m->Cg, m->rounds, RATE_OF_FIRE);
#endif
...
#define TICK_SIZE	(RETICLE_SIZE + 12)
	...
	VDrawArc(u->v, xt1, yt1, size1, size1,
		 90 * 64, arc, u->v->pixel[HUDPixel]);
    }
    VDrawSegments(u->v, seg, nseg, u->v->pixel[HUDPixel]);
...
/*  Step 6  */
    ...
    tmp1 = tmp;
    x = (u->v->Middl.x + (int) (tmp1.y * u->v->Scale.x / tmp1.x)) >> 2;
    y = (u->v->Middl.y + (int) (tmp1.z * u->v->Scale.y / tmp1.x)) >> 2;
...
#define FT_TO_M(x)     (0.3048*(x))
		worldLocation[0] = FT_TO_M(r->Sg.x);
		worldLocation[1] = FT_TO_M(r->Sg.y);
		worldLocation[2] = FT_TO_M(r->Sg.z);
