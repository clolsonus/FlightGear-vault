...
#endif
{
  ...
  #elif defined(WIN32)
  {
    if (!fg_hdc) fg_hdc=GetDC(fg_hwnd);
    fg_hpalette=CreatePalette(&fg_logpalette);
    SetPaletteEntries(fg_hpalette,0,NUM_COLORS,PaletteArray);
    SelectPalette(fg_hdc,fg_hpalette,FALSE);
    RealizePalette(fg_hdc);
  }  
  #endif
}
  ...
  if (RGBColor->r | RGBColor->g | RGBColor->b) {
        ...
        #elif defined(USE_FB)
	{
	  ...
	  ioctl(fb_fd,FBIOPUTCMAP,&fb_cmap);
        ...
        #elif defined(WIN32)
        if (SetPaletteEntries(fg_hpalette,i,1,&(PaletteArray[i])))
        {
          RealizePalette(fg_hdc);
        } else {
          i=(fg_find_closest_color(RGBColor));
