...
void *FG_message_loop(void *nothing)
{
  MSG msg;
...
LRESULT CALLBACK FG_WndProc(HWND hwnd, UINT msg, WPARAM wparam, LPARAM lparam)
{
  ...
  switch (msg) {
    case WM_CREATE:
      {
        ...
        bitmap.bmHeight=scn_height;
        bitmap.bmWidth=scn_width;
        bitmap.bmWidthBytes=scn_width;
        bitmap.bmPlanes=1;
        bitmap.bmBitsPixel=8;
        bitmap.bmBits=display_bitmaps[0]->lines[0];
        ...
        SetBitmapBits(fg_hbitmap,scn_width*scn_height,display_bitmaps[0]->lines[0]);
    ...
    case WM_PAINT:
      {
        ...
        SetBitmapBits(fg_hbitmap,
          display_bitmaps[0]->w*display_bitmaps[0]->h,
    ...
    case WM_CHAR:
      {
        int code=fg_GetWinKey(wparam,key_flags);
        if (code) {
